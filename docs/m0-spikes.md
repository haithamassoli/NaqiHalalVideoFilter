...
| `htdemucs_f16.onnx` | 87.3 MB | `74bd4de2` | facebookresearch/demucs weights via sevagh/demucs.onnx export | MIT |
...
**NudeNet 320n** — upstream ONNX used unmodified (reference *is* this artifact). Input `images` [1,3,320,320] f32 RGB /255; preprocessing locked from upstream source: pad right/bottom to square, resize 320, `blobFromImage(swapRB=true)`; output `output0` [1,22,2100] = 4 box rows (cx,cy,w,h in 320-space) + 18 class rows; upstream thresholds 0.2 keep / 0.25 NMS score / 0.45 IoU; `FACE_FEMALE`=1, `FACE_MALE`=12. Face-crop validation carried to M1 QA (needs sample crops).

**htdemucs f16** — exported with torch 2.13 (dynamo exporter, opset 18) from sevagh/demucs.onnx's fork (STFT/iSTFT outside the graph; one-line patch making its `diffq` import optional). IO: `input` [1,2,343980] waveform + `x` [1,4,2048,336] cac spectrogram (nfft 4096, hop 1024, 7.8 s @ 44.1 kHz) → masked spec [1,4,4,2048,336] + time branch [1,4,2,343980]; stems = istft(spec) + time branch, outside the graph (M2 driver). Parity vs the torch reference on a 7.8 s synthetic segment: f32 75.2/89.0 dB SNR (spec/wave) — conversion exact; f16 61.5/65.9 dB — expected f16 loss, fine for masking. Full 30 s overlap-add **stems** parity lands with the M2 chunk driver.
