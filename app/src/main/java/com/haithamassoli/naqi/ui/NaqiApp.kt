...
fun NaqiApp(modifier: Modifier = Modifier) {
    ...
    // Process death loses even saved state, so re-attach to a live job on a cold start — otherwise a
    // share leaves the user staring at the pick screen with no sign that anything happened.
